...
static int64_t _JJDiskSpaceFree() {
    NSError *error = nil;
...
static void _JJDiskCacheInitGlobal() {
...
static JJDiskCache *_JJDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    
    _JJDiskCacheInitGlobal();
...
static void _JJDiskCacheSetGlobal(JJDiskCache *cache) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _JJDiskSpaceFree();
...
- (instancetype)initWithPath:(NSString *)path {
    return [self initWithPath:path inlineThreshold:1024 * 20];  // 20 KB
}

- (instancetype)initWithPath:(NSString *)path inlineThreshold:(NSUInteger)threshold {
    self = [super init];
    if (self) {
        JJDiskCache *globalCache = _JJDiskCacheGetGlobal(path);
        if (globalCache) return globalCache;
        
        JJKVStorageType type;
        if (threshold == 0) {
            type = JJKVStorageTypeFile;
        } else if (threshold == NSUIntegerMax) {
            type = JJKVStorageTypeSQLite;
        } else {
            type = JJKVStorageTypeMixed;
        }
        
        JJKVStorage *kv = [[JJKVStorage alloc] initWithPath:path type:type];
        if (!kv) return nil;
        
        _kv = kv;
        _path = path;
        _lock = dispatch_semaphore_create(1);
        _queue = dispatch_queue_create("com.jj.cache.disk", DISPATCH_QUEUE_CONCURRENT);
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [JJDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    if (_customArchiveBlock) {
        ...
        @try {
            if (@available(iOS 11.0, *)) {
                value = [NSKeyedArchiver archivedDataWithRootObject:object requiringSecureCoding:NO error:NULL];
            } else {
    ...
    if (!value) return;
    NSString *fileName = nil;
    if (_kv.type != JJKVStorageTypeSQLite) {
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    JJKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (_customUnarchiveBlock) {
        ...
        @try {
            if (@available(iOS 11.0, *)) {
                object = [NSKeyedUnarchiver unarchivedObjectOfClass:[object class] fromData:object error:NULL];
            } else {
    ...
    if (object && item.extendedData) {
        [JJDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
