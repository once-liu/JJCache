...
static UIApplication *_JJSharedApplication() {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if ([[[NSBundle mainBundle] bundlePath] hasPrefix:@".appex"]) isAppExtension = YES;
    });
...
@implementation JJKVStorageItem
...
- (BOOL)_dbClose {
    ...
    do {
        ...
        if (result == SQLITE_OK || result == SQLITE_LOCKED) {
            if (!stmtFinalized) {
                stmtFinalized = YES;
                sqlite3_stmt *stmt;
...
- (BOOL)_dbDeleteItemWithKeys:(NSArray *)keys {
    ...
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    int result = sqlite3_bind_text(stmt, 1, sql.UTF8String, -1, NULL);
    if (result !=SQLITE_OK) {
        if (_errorLogsEnabled) NSLog(@"%s line:%d db delete error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_db));
        return NO;
    }
...
- (JJKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    JJKVStorageItem *item = [JJKVStorageItem new];
...
- (JJKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    JJKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            JJKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (BOOL)_fileMoveAllToTrash {
    ...
    BOOL success = [[NSFileManager defaultManager] moveItemAtPath:_dataPath toPath:tmpPath error:NULL];
    if (success) {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"JJKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:(JJKVStorageTypeFile)];
}

- (instancetype)initWithPath:(NSString *)path type:(JJKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        return nil;
    }
    if (type > JJKVStorageTypeMixed) {
        return nil;
    }
    ...
    if (self) {
        ...
        _trashQueue = dispatch_queue_create("com.jj.cache.disk.trash", DISPATCH_QUEUE_SERIAL);
        ...
        NSError *error = nil;
        // 创建 path、dataPath、trashPath 路径失败
        if (![[NSFileManager defaultManager] createDirectoryAtPath:path withIntermediateDirectories:YES attributes:nil error:&error] ||
            ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kDataDirectoryName] withIntermediateDirectories:YES attributes:nil error:&error] ||
            ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName] withIntermediateDirectories:YES attributes:nil error:&error]) {
            NSLog(@"JJKVStorage init error: %@", error);
            return nil;
        }
...
- (void)dealloc {
    UIBackgroundTaskIdentifier taskID = [_JJSharedApplication() beginBackgroundTaskWithExpirationHandler:^{}];
    [self _dbClose];
    if (taskID != UIBackgroundTaskInvalid) {
        [_JJSharedApplication() endBackgroundTask:taskID];
    }
}
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value fileName:(NSString *)fileName extendedData:(NSData *)extendedData  {
    ...
    // 写入文件必须要有 fileName
    if (_type == JJKVStorageTypeFile && fileName.length == 0) {
        return NO;
    }
    ...
    if (fileName.length) {
        ...
        if (_type != JJKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case JJKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        }
            break;
        case JJKVStorageTypeFile:
        case JJKVStorageTypeMixed: {
            NSString *fileName = [self _dbGetFilenameWithKey:key];
            if (fileName) {
                [self _fileDeleteWithName:fileName];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (JJKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (JJKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    JJKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (JJKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    JJKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
